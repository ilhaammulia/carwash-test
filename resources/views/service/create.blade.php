...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Create Service | Carwash</title>
            ...
            <div class="min-h-screen flex flex-col sm:flex-row antialiased text-gray-800">
                ...
                <div class="w-full p-8 bg-gray-50">
                    <div class="w-full mt-8">
                        <x-alert />
                        <form action="{{ route('service.store') }}" method="post" class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                        @csrf
                          <!-- Header -->
                          ...
                          <div class="flex flex-col gap-4 mt-2 space-y-3 p-4">
                              <input type="text" name="name" class="py-4 px-3 pe-11 block w-full border-gray-500 shadow-sm text-sm rounded-lg disabled:opacity-50 disabled:pointer-events-none" placeholder="Service Name">
                              <input type="text" name="price" class="py-4 px-3 pe-11 block w-full border-gray-500 shadow-sm text-sm rounded-lg disabled:opacity-50 disabled:pointer-events-none" placeholder="Price">
                          </div>
                          <!-- End Form -->
